...
export default function AdminFlags({ notify }) {
  ...
  return (
    <div style={{ backgroundColor: 'white', border: '1.5px solid #e2e8f0', borderRadius: '12px', padding: '14px', marginBottom: '14px' }}>
      ...
      {uncreated.length > 0 && (
        <div style={{ backgroundColor: '#f7fafc', border: '1px solid #edf2f7', borderRadius: '10px', padding: '10px', marginBottom: '12px' }}>
          <div style={{ fontSize: '0.72rem', fontWeight: '700', color: '#718096', marginBottom: '7px' }}>NEW FLAG</div>
          <select value={selectedNew} onChange={(e) => setSelectedNew(e.target.value)}
            style={{ width: '100%', boxSizing: 'border-box', padding: '9px 11px', borderRadius: '9px', border: '1.5px solid #e2e8f0', fontSize: '0.84rem', color: '#2d3748', backgroundColor: 'white', marginBottom: '8px' }}>
      ...
      {loading ? (
        ...
        <div style={{ fontSize: '0.75rem', color: '#a0aec0' }}>No flags yet.</div>
      ) : created.map((f) => {
          ...
          <div key={f.key} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '8px', alignItems: 'center' }}>
              <div style={{ minWidth: 0 }}>
                <div style={{ fontSize: '0.8rem', fontWeight: '700', color: '#2d3748' }}>
                  <span style={{ color: blocking ? '#c53030' : '#059669' }}>● </span>{f.label}
                </div>
                <div style={{ fontSize: '0.68rem', color: '#a0aec0', marginTop: '2px' }}>{f.description}</div>
              </div>
              <div style={{ display: 'flex', gap: '5px', flexShrink: 0 }}>
                <button onClick={() => toggle(f)} disabled={busyKey === f.key}
                  style={{
                    ...
                    backgroundColor: 'white', color: blocking ? '#c53030' : '#276749',
                ...
                <button onClick={() => remove(f)} disabled={busyKey === f.key}
                  style={{ padding: '6px 9px', borderRadius: '7px', border: '1.5px solid #e2e8f0', backgroundColor: 'white', color: '#a0aec0', fontSize: '0.7rem', fontWeight: '700', cursor: 'pointer' }}>
                  Remove
                </button>
              </div>
            </div>
