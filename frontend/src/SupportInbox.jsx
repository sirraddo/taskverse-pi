...
function NewTicketForm({ onCreated, onCancel }) {
  ...
  const inputStyle = {
    ...
    border: '1.5px solid var(--border)', fontSize: '0.86rem', color: 'var(--text-secondary)',
    backgroundColor: 'var(--surface)', outline: 'none', marginBottom: '10px', fontFamily: 'inherit',
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        <button onClick={onCancel} style={{ flex: 1, padding: '11px', borderRadius: '10px', border: '1.5px solid var(--border)', backgroundColor: 'var(--surface)', color: 'var(--text-muted)', fontWeight: '700', cursor: 'pointer', fontSize: '0.85rem' }}>
          Cancel
        </button>
...
function TicketThread({ ticketId, onBack }) {
  ...
  if (!ticket) return <div style={{ fontSize: '0.82rem', color: 'var(--text-faintest)' }}>Loading…</div>;

  const badge = STATUS_BADGE[ticket.status];

  return (
    <div>
      <button onClick={onBack} style={{ background: 'var(--surface)', border: '1px solid var(--border-strong)', padding: '6px 14px', borderRadius: '8px', cursor: 'pointer', fontSize: '0.82rem', color: 'var(--text-muted)', fontWeight: '600', marginBottom: '14px' }}>
        ← All tickets
      </button>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '4px' }}>
        <h3 style={{ margin: 0, fontSize: '0.98rem', color: 'var(--text)' }}>{ticket.subject}</h3>
      ...
      {ticket.refId && (
        <div style={{ fontSize: '0.7rem', color: 'var(--text-faintest)', fontFamily: 'monospace', marginBottom: '12px' }}>Ref: {ticket.refId}</div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '9px', marginBottom: '14px' }}>
        {ticket.messages.map((m, i) => (
          <div key={i} style={{
            ...
            <div style={{ fontSize: '0.66rem', fontWeight: '700', color: m.from === 'admin' ? '#276749' : 'var(--text-faint)', marginBottom: '3px' }}>
              {m.from === 'admin' ? 'Support' : 'You'} · {new Date(m.createdAt).toLocaleString()}
            </div>
            <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', whiteSpace: 'pre-wrap', lineHeight: 1.4 }}>{m.body}</div>
          </div>
        ))}
      </div>

      {ticket.status === 'closed' && (
        <div style={{ fontSize: '0.78rem', color: 'var(--text-faintest)', fontStyle: 'italic', marginBottom: '8px' }}>
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        <input value={reply} onChange={(e) => setReply(e.target.value)}
          ...
          style={{ flex: 1, padding: '10px 12px', borderRadius: '10px', border: '1.5px solid var(--border)', fontSize: '0.85rem', color: 'var(--text-secondary)', backgroundColor: 'var(--surface)' }} />
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif', maxWidth: '500px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '18px' }}>
        ...
        <h2 style={{ margin: 0, fontSize: '1.05rem', color: 'var(--text)' }}>🎧 Support</h2>
      </div>

      {typeof view === 'string' && view.startsWith('ticket:') ? (
        <TicketThread ticketId={view.slice(7)} onBack={() => { setView('list'); load(); }} />
      ) : view === 'new' ? (
        <div style={{ backgroundColor: 'var(--surface)', borderRadius: '14px', padding: '16px', boxShadow: '0 2px 10px var(--shadow-color)' }}>
          ...
          {tickets === null ? (
            ...
            return (
              <div key={t.id} onClick={() => setView(`ticket:${t.id}`)}
                style={{ backgroundColor: 'var(--surface)', borderRadius: '12px', padding: '13px 14px', marginBottom: '8px', boxShadow: '0 1px 4px var(--shadow-color)', cursor: 'pointer' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <div style={{ fontWeight: '700', color: 'var(--text-secondary)', fontSize: '0.88rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                      {t.subject}
                      {t.hasUnreadForUser && <span style={{ width: '7px', height: '7px', borderRadius: '50%', backgroundColor: '#e53e3e', flexShrink: 0 }} />}
                    </div>
                    <div style={{ fontSize: '0.76rem', color: 'var(--text-faintest)', marginTop: '3px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {t.lastMessage}
                    </div>
                  </div>
