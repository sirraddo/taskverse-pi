      ...
      {view === 'feed' && (
        <div style={{ padding: '16px' }}>
          ...
          {filtered.map(task => {
            ...
            const totalSlots = task.slotsLeft + slotsFilled;
            ...
            return (
              <div key={task.id} style={{ backgroundColor: 'white', borderRadius: '14px', padding: '14px 14px 12px', marginBottom: '10px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', opacity: isFull ? 0.65 : 1, animation: 'fadeUp 0.3s ease' }}>
                ...
                <button onClick={() => { setSelectedTask(task); setView('submit'); }} disabled={isFull}
                  style={{ width: '100%', backgroundColor: isFull ? '#edf2f7' : '#667eea', color: isFull ? '#a0aec0' : 'white', border: 'none', padding: '10px', borderRadius: '10px', cursor: isFull ? 'not-allowed' : 'pointer', fontWeight: '700', fontSize: '0.84rem' }}>
                  {isFull ? 'Task Full' : t.openTask + ' →'}
                </button>
              </div>
            );
          })}
          ...
          <div style={{ display: 'flex', gap: '8px', marginTop: '14px' }}>
            <button onClick={() => setScreen('leaderboard')} style={{ flex: 1, padding: '11px', backgroundColor: 'white', border: '1.5px solid #e2e8f0', borderRadius: '12px', cursor: 'pointer', fontWeight: '700', color: '#667eea', fontSize: '0.82rem', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>🏆 Leaderboard</button>
            <button onClick={() => setScreen('history')}     style={{ flex: 1, padding: '11px', backgroundColor: 'white', border: '1.5px solid #e2e8f0', borderRadius: '12px', cursor: 'pointer', fontWeight: '700', color: '#667eea', fontSize: '0.82rem', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>📜 My History</button>
          </div>
